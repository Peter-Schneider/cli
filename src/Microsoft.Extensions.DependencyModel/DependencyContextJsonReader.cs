    ...
    public class DependencyContextJsonReader : IDependencyContextReader
    {
        ...
        private Library ReadLibrary(JProperty property, bool runtime, Dictionary<string, LibraryStub> libraryStubs)
        {
            ...
            if (runtime)
            {
                ...
                var runtimeAssemblies = new Dictionary<string, IReadOnlyList<RuntimeAssembly>>();
                var nativeLibraries = new Dictionary<string, IReadOnlyList<string>>();
                foreach (var ridGroup in entries.GroupBy(e => e.Rid))
                {
                    ...
                    {
                        runtimeAssemblies[ridGroup.Key] = runtimeAssets;
                    }
                    ...
                    if(nativeAssets.Any())
                    {
                        nativeLibraries[ridGroup.Key] = nativeAssets;
                    }
                }

                runtimeAssemblies[string.Empty] = ReadAssetList(libraryObject, DependencyContextStrings.RuntimeAssembliesKey)
                    .Select(RuntimeAssembly.Create)
                    .ToArray();

                nativeLibraries[string.Empty] = ReadAssetList(libraryObject, DependencyContextStrings.NativeLibrariesKey);
                ...
                return new RuntimeLibrary(
                    ...
                    version: version,
                    hash: stub.Hash,
                    assemblies: new RuntimeAssetCollection<RuntimeAssembly>(runtimeAssemblies),
                    nativeLibraries: new RuntimeAssetCollection<string>(nativeLibraries),
